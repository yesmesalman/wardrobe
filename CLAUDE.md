...
### How the 3D works
- Blank models live in `src/assets/models/*.glb` (short-sleeve shirt, long-sleeve shirt, long pants, shorts; the pants' legs overlap at the top so there is no crotch pouch, and long pants have a long inseam and hem), generated by `npm run generate:models` (`scripts/generate-models.js`). Each carries a `decal` anchor in its node extras.
