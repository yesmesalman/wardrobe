...
- The user's photo is projected onto the model as a decal inside a WebView (`webview/scene.js`), bundled by `npm run build:webview` into `src/webview/sceneHtml.ts` (generated, committed). Re-run both scripts after changing a model or the scene.
- Library cards show a JPEG snapshot of the 3D garment taken when it is saved; only the detail/add screens run a live WebView.
